...
const SECTIONS = [
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'h2', text: 'Slate 7 — Previous Primary Router (now AP only)' },
      { type: 'table2', rows: [
        ['Wi-Fi',    'Dual-band Wi-Fi 7'],
        ['Ethernet', 'Dual 2.5G'],
        ['Tailscale','Was running natively — AP mode only now'],
        ['IP',       'YOUR_HOST_IP'],
        ['Role',     'Wi-Fi access point only — no longer routing or doing NAT'],
      ]},
      { type: 'h2', text: 'Brume 2 — WAN Gateway / Port Forward Box' },
      { type: 'table2', rows: [
        ['Role',      'WAN gateway — NAT + port forwards for game ports → GameStack Host'],
        ['WAN IP',    'YOUR_HOST_IP'],
        ['LAN IP',    'YOUR_HOST_IP'],
        ['Subnet',    '192.168.8.x — GameStack Host, Slate 7 AP, Switch'],
        ['Port Fwds', 'Game ports → YOUR_HOST_IP'],
        ['Notes',     'Asus has static route to 192.168.8.x via Brume 2 WAN — allows 192.168.1.x devices to reach GameStack'],
      ]},
      { type: 'h2', text: 'Slate 7 — Wi-Fi Access Point' },
      { type: 'table2', rows: [
        ['Role',    'Wi-Fi AP — wired to switch, provides wireless on 192.168.8.x subnet'],
        ['IP',      'YOUR_SLATE7_IP (AP — no fixed IP needed)'],
      ]},
  ...
  },
  {
    ...
    content: [
      { type: 'table2', rows: [
        ...
        ['Topology',      'Double NAT: Internet → ISP Modem → Asus Router (192.168.1.x) → Brume 2 (192.168.8.x) → Switch → GameStack Host'],
        ['DMZ/Route',     'Asus has static route to 192.168.8.x via Brume 2 WAN IP. No DMZ — clean double NAT.'],
        ['Port Forwards', 'Game ports forwarded on Brume 2 → YOUR_HOST_IP (YOUR_HOST_IP)'],
  ...
  },
  {
    ...
    content: [
      ...
      { type: 'bullets', items: [
        ...
        { status: 'done', text: 'Wolf UI working — was broken by unreliable from-source Moonlight build. Play Store version resolves both UI launch and audio.' },
      ...
      { type: 'bullets', items: [
        ...
        { status: 'done', text: 'Network topology finalised — double NAT via Brume 2, Asus static route to 192.168.8.x, port forwards on Brume 2' },
        { status: 'done', text: 'UDP game port forwards added to Brume 2 → YOUR_HOST_IP (YOUR_HOST_IP)' },
  ...
  },
  {
    ...
    content: [
      { type: 'table3', headers: ['Item', 'Details', 'Config Changes Required'], rows: [
        ...
        ['Router swap',      'Complete — Brume 2 is WAN gateway, GameStack Host wired to switch',   'Done — port forwards on Brume 2, static route on Asus'],
  ...
  },
  {
    ...
    content: [
      { type: 'bullets', items: [
        ...
        { text: 'Network is double NAT: Internet → ISP Modem → Asus Router (192.168.1.x) → Brume 2 (192.168.8.x) → Switch → GameStack Host (YOUR_HOST_IP)' },
        { text: 'Asus has static route to 192.168.8.x via Brume 2 WAN IP — no DMZ needed. Port forwards live on Brume 2.' },
  ...
  },
  {
    ...
    content: [
      { type: 'bullets', items: [
        ...
        { status: 'done', text: 'Router 2 transition complete — K8 Plus (GameStack Host) wired to switch via Brume 2' },
      ]},
    ],
  },
];
